...
public:
  ...
  ~VoxelOctoTree()
  {
    ...
    std::vector<int>().swap(layer_init_num_);
  }
...
public:
  ...
  // Reused buffers for BuildResidualListOMP().
  // Avoid allocating large temporary vectors in every LIO iteration.
  std::vector<PointToPlane> all_ptpl_list_cache_;
  std::vector<unsigned char> useful_ptpl_cache_;
  std::vector<size_t> index_cache_;
  ...
  ~VoxelMapManager()
  {
    ...
    PointCloudXYZI().swap(*feats_undistort_);
    PointCloudXYZI().swap(*feats_down_body_);
    PointCloudXYZI().swap(*feats_down_world_);

    std::vector<M3D>().swap(cross_mat_list_);
    std::vector<M3D>().swap(body_cov_list_);
    std::vector<pointWithVar>().swap(pv_list_);
    std::vector<PointToPlane>().swap(ptpl_list_);
    std::vector<PointToPlane>().swap(all_ptpl_list_cache_);
    std::vector<unsigned char>().swap(useful_ptpl_cache_);
    std::vector<size_t>().swap(index_cache_);
  }
