...
namespace {
    double g_opt_trim_start_sec = 0.0;
    double g_opt_trim_end_sec   = 0.0;
}
...
optimization::optimization(ros::NodeHandle &nh)
{
    ...
    nh.param<string>("gps/gps_topic", gps_topic, "/receiver_pvt");
    ...
    //   trim_start_sec: 10.0
    //   trim_end_sec: 8.0
    nh.param<double>("opt/trim_start_sec", g_opt_trim_start_sec, 0.0);
    nh.param<double>("opt/trim_end_sec",   g_opt_trim_end_sec,   0.0);
...
void optimization::offlineOptimizationTask() {
    ...
    // ---------------------------------------------------------------------
    // 剔除开头/结尾污染阶段
    // 目的：去掉采集器刚拿起、结束放回地面等非正常运动段，避免污染初始对齐和图优化。
    // 注意：这里同时裁剪 FAST-LIVO2 关键帧、关键帧点云、initialEstimate 和 RTK 队列，
    //      保证后续 initialAlign()、buildBatchGraph()、TUM 输出的数据时间范围一致。
    // ---------------------------------------------------------------------
    auto trimTrajectoryByTime = [&]() {
        if (g_opt_trim_start_sec <= 0.0 && g_opt_trim_end_sec <= 0.0) {
            ROS_INFO("[trimTrajectoryByTime] disabled. trim_start_sec=%.3f, trim_end_sec=%.3f",
                     g_opt_trim_start_sec, g_opt_trim_end_sec);
            return;
        }

        if (cloudKeyPoses6D->empty()) {
            ROS_WARN("[trimTrajectoryByTime] cloudKeyPoses6D is empty, skip trimming.");
            return;
        }

        if (surfCloudKeyFrames.size() != cloudKeyPoses6D->size()) {
            ROS_WARN("[trimTrajectoryByTime] surfCloudKeyFrames size (%lu) != cloudKeyPoses6D size (%lu). skip trimming to avoid index mismatch.",
                     surfCloudKeyFrames.size(), cloudKeyPoses6D->size());
            return;
        }

        const double t0 = cloudKeyPoses6D->points.front().time;
        const double t1 = cloudKeyPoses6D->points.back().time;
        const double valid_start = t0 + std::max(0.0, g_opt_trim_start_sec);
        const double valid_end   = t1 - std::max(0.0, g_opt_trim_end_sec);

        if (valid_start >= valid_end) {
            ROS_WARN("[trimTrajectoryByTime] invalid trim range: valid_start=%.6f, valid_end=%.6f. skip trimming.",
                     valid_start, valid_end);
            return;
        }

        pcl::PointCloud<PointTypePose>::Ptr newKeyPoses6D(new pcl::PointCloud<PointTypePose>());
        std::vector<PointCloudXYZRGB::Ptr> newSurfCloudKeyFrames;
        gtsam::Values newInitialEstimate;

        int new_idx = 0;
        for (size_t i = 0; i < cloudKeyPoses6D->size(); ++i) {
            const double t = cloudKeyPoses6D->points[i].time;
            if (t < valid_start || t > valid_end) {
                continue;
            }

            PointTypePose pose = cloudKeyPoses6D->points[i];
            pose.intensity = new_idx;
            newKeyPoses6D->push_back(pose);
            newSurfCloudKeyFrames.push_back(surfCloudKeyFrames[i]);

            if (initialEstimate.exists(i)) {
                gtsam::Pose3 oldPose = initialEstimate.at<gtsam::Pose3>(i);
                newInitialEstimate.insert(new_idx, oldPose);
            } else {
                ROS_WARN("[trimTrajectoryByTime] initialEstimate missing key %lu, skip this pose.", i);
                newKeyPoses6D->points.pop_back();
                newSurfCloudKeyFrames.pop_back();
                continue;
            }
        ...
        if (newKeyPoses6D->empty()) {
            ROS_WARN("[trimTrajectoryByTime] all keyframes were trimmed, keep original data.");
            return;
        }

        decltype(gpsQueue) newGpsQueue;
        for (const auto& gps : gpsQueue) {
            const double t = gps.header.stamp.toSec();
            if (t >= valid_start && t <= valid_end) {
                newGpsQueue.push_back(gps);
            }
        }

        if (newGpsQueue.empty()) {
            ROS_WARN("[trimTrajectoryByTime] all RTK data were trimmed, keep original data.");
            return;
        }

        const size_t old_pose_size = cloudKeyPoses6D->size();
        const size_t old_gps_size  = gpsQueue.size();

        cloudKeyPoses6D = newKeyPoses6D;
        surfCloudKeyFrames = newSurfCloudKeyFrames;
        initialEstimate = newInitialEstimate;
        gpsQueue = newGpsQueue;

        ROS_INFO("[trimTrajectoryByTime] trim_start_sec=%.3f, trim_end_sec=%.3f",
                 g_opt_trim_start_sec, g_opt_trim_end_sec);
        ROS_INFO("[trimTrajectoryByTime] valid time range: %.6f -> %.6f",
                 valid_start, valid_end);
        ROS_INFO("[trimTrajectoryByTime] keyframes: %lu -> %lu, RTK: %lu -> %lu",
                 old_pose_size, cloudKeyPoses6D->size(), old_gps_size, gpsQueue.size());
    };

    trimTrajectoryByTime();
...
void optimization::buildBatchGraph()
{
    ...
    ROS_INFO("rtk_cov(default var), livo2_RPY_cov, livo2_XYZ_cov: %f, %f, %f",
             rtk_cov, livo2_RPY_cov, livo2_XYZ_cov);

    gtsam::noiseModel::Diagonal::shared_ptr priorNoise =
        gtsam::noiseModel::Diagonal::Variances(
            (gtsam::Vector(6) << 1e-6, 1e-6, 1e-6,
                                  1e-6, 1e-6, 1e-6).finished());
    ...
    for (size_t i = 0; i < initialEstimate.size(); ++i)
    {
        ...
        else
        {
            ...
            gtSAMgraph.add(
                gtsam::BetweenFactor<gtsam::Pose3>(
                    i - 1, i, T_rel_ant_from_svd, odometryNoise));
        }

        if (i >= gpsQueue_B.size())
        {
            ROS_WARN("[buildBatchGraph] gpsQueue_B size is smaller than initialEstimate size. skip GPS factor at index %lu", i);
            continue;
        }

        const auto& it = gpsQueue_B[i];
        ...
        {
            ROS_WARN("[buildBatchGraph] invalid GPS position at index %lu, skip GPS factor.", i);
            continue;
        }

        gtsam::Point3 gps_point(x, y, z);

        // h_acc / v_acc 一般表示标准差级别的定位精度，单位 m
        double sigma_xy = it.pose.covariance[0];   // h_acc
        double sigma_z  = it.pose.covariance[14];  // v_acc

        // 如果 gpsQueue_B 里没有有效协方差，则回退到 yaml 中的 rtk_cov
        // 注意：rtk_cov 在你原代码里是 Variance，所以这里取 sqrt 作为 sigma
        const double fallback_sigma_xy = std::sqrt(std::max(rtk_cov, 1e-8));
        const double fallback_sigma_z  = 1.0;

        if (!std::isfinite(sigma_xy) || sigma_xy <= 0.0)
        {
            sigma_xy = fallback_sigma_xy;
        }

        if (!std::isfinite(sigma_z) || sigma_z <= 0.0)
        {
            sigma_z = fallback_sigma_z;
        }

        // 设置上下限，避免某些 RTK 报得过小/过大导致优化不稳定
        // RTK Fixed 室外常见：h_acc 0.02~0.05m，v_acc 0.05~0.15m
        sigma_xy = std::max(sigma_xy, 0.02);
        sigma_z  = std::max(sigma_z,  0.05);

        sigma_xy = std::min(sigma_xy, 1.0);
        sigma_z  = std::min(sigma_z,  2.0);

        gtsam::noiseModel::Diagonal::shared_ptr dynamic_gps_noise =
            gtsam::noiseModel::Diagonal::Sigmas(
                (gtsam::Vector(3) << sigma_xy, sigma_xy, sigma_z).finished());

        gtSAMgraph.add(gtsam::GPSFactor(i, gps_point, dynamic_gps_noise));

        if (i % 100 == 0)
        {
            ROS_INFO("[buildBatchGraph] GPS factor %lu: sigma_xy=%.4f m, sigma_z=%.4f m",
                     i, sigma_xy, sigma_z);
        }
    }
}
