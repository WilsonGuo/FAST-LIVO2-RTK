...
void VoxelMapManager::BuildResidualListOMP(std::vector<pointWithVar> &pv_list, std::vector<PointToPlane> &ptpl_list)
{
  ...
  // Reuse buffers instead of allocating large vectors in every LIO iteration.
  all_ptpl_list_cache_.resize(pv_list.size());
  useful_ptpl_cache_.assign(pv_list.size(), 0);
  index_cache_.resize(pv_list.size());
  ...
  #endif
  for (int i = 0; i < static_cast<int>(index_cache_.size()); i++)
  {
    ...
    if (iter != voxel_map_.end())
    {
      ...
      if (is_sucess)
      {
        useful_ptpl_cache_[i] = 1;
        all_ptpl_list_cache_[i] = single_ptpl;
      }
    }
  }

  for (size_t i = 0; i < useful_ptpl_cache_.size(); i++)
  {
    if (useful_ptpl_cache_[i])
    {
      ptpl_list.push_back(all_ptpl_list_cache_[i]);
    }
  }
}
