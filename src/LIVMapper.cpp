...
void LIVMapper::readParameters(ros::NodeHandle &nh)
{
  ...
  // GLOBAL_LVBA_EXPORT_V1
  nh.param<bool>("global_lvba/output_en", global_lvba_output_en, false);
  nh.param<string>("global_lvba/sequence_name", global_lvba_sequence_name, "sequence");

  ROS_WARN(
      "[Global-LVBA] parameter: output_en=%s, sequence=%s, nh=%s",
      global_lvba_output_en ? "true" : "false",
      global_lvba_sequence_name.c_str(),
...
{
  // ============================================================
  // Global-LVBA output initialization
  // 独立于 PCD / COLMAP 输出
  // ============================================================
  if (global_lvba_output_en)
  {
    global_lvba_output_dir =
        std::string(ROOT_DIR) +
        "Log/GlobalLVBA/" +
        global_lvba_sequence_name;

    const std::string image_dir =
        global_lvba_output_dir + "/all_image";

    const std::string pcd_dir =
        global_lvba_output_dir + "/all_pcd_body";

    ROS_WARN(
        "[Global-LVBA] output_en=true, sequence=%s",
        global_lvba_sequence_name.c_str());

    ROS_WARN(
        "[Global-LVBA] output_dir=%s",
        global_lvba_output_dir.c_str());
    ...
    {
      ROS_ERROR(
          "[Global-LVBA] Failed to create output directories.");
    }
    else
    {
      fout_lvba_image_pose.open(
          image_dir + "/image_poses.txt",
          std::ios::out);

      fout_lvba_lidar_pose.open(
          pcd_dir + "/lidar_poses.txt",
          std::ios::out);

      if (!fout_lvba_image_pose.is_open())
      {
        ROS_ERROR(
            "[Global-LVBA] Failed to open image_poses.txt");
      }

      if (!fout_lvba_lidar_pose.is_open())
      {
        ROS_ERROR(
            "[Global-LVBA] Failed to open lidar_poses.txt");
      }

      if (fout_lvba_image_pose.is_open() &&
          fout_lvba_lidar_pose.is_open())
      {
        ROS_INFO(
            "[Global-LVBA] Dataset output initialized successfully.");
      }
    }
  }

  // ============================================================
  // Existing COLMAP initialization
  // ============================================================
  ...
  if (colmap_output_en)
  {
    fout_points.open(
        std::string(ROOT_DIR) +
            "Log/Colmap/sparse/0/points3D.txt",
        std::ios::out);
  }
  ...
  if (pcd_save_interval > 0)
  {
    fout_pcd_pos.open(
        std::string(ROOT_DIR) +
            "Log/PCD/scans_pos.json",
        std::ios::out);
  }

  fout_pre.open(
      DEBUG_FILE_DIR("mat_pre.txt"),
      std::ios::out);

  fout_out.open(
      DEBUG_FILE_DIR("mat_out.txt"),
      std::ios::out);
}

void LIVMapper::initializeSubscribersAndPublishers(ros::NodeHandle &nh, image_transport::ImageTransport &it)
{
  sub_pcl = p_pre->lidar_type == AVIA ? nh.subscribe(lid_topic, 5, &LIVMapper::livox_pcl_cbk, this) : nh.subscribe(lid_topic, 5, &LIVMapper::standard_pcl_cbk, this);
  sub_imu = nh.subscribe(imu_topic, 1000, &LIVMapper::imu_cbk, this);
  sub_img = nh.subscribe(img_topic, 5, &LIVMapper::img_cbk, this);

  subGPS_pvt = nh.subscribe<gnss_comm::GnssPVTSolnMsg>("/receiver_pvt", 20, &LIVMapper::rtk_cbk, this);

  pub_odom = nh.advertise<nav_msgs::Odometry>("/odometry/fast_livo2", 10);
  pub_lidarRGB = nh.advertise<sensor_msgs::PointCloud2>("/synced_cloud", 2);
  pub_rtk = nh.advertise<nav_msgs::Odometry>("/gps/odometry", 10);

  pubLaserCloudFullRes = nh.advertise<sensor_msgs::PointCloud2>("/cloud_registered", 2);
  pubNormal = nh.advertise<visualization_msgs::MarkerArray>("visualization_marker", 2);
  pubSubVisualMap = nh.advertise<sensor_msgs::PointCloud2>("/cloud_visual_sub_map_before", 2);
  pubLaserCloudEffect = nh.advertise<sensor_msgs::PointCloud2>("/cloud_effected", 2);
  pubLaserCloudMap = nh.advertise<sensor_msgs::PointCloud2>("/Laser_map", 2);
  ...
  pubLaserCloudDyn = nh.advertise<sensor_msgs::PointCloud2>("/dyn_obj", 2);
  pubLaserCloudDynRmed = nh.advertise<sensor_msgs::PointCloud2>("/dyn_obj_removed", 2);
  pubLaserCloudDynDbg = nh.advertise<sensor_msgs::PointCloud2>("/dyn_obj_dbg_hist", 2);
  mavros_pose_publisher = nh.advertise<geometry_msgs::PoseStamped>("/mavros/vision_pose/pose", 10);
  pubImage = it.advertise("/rgb_img", 1);
  pubImuPropOdom = nh.advertise<nav_msgs::Odometry>("/LIVO2/imu_propagate", 100);
  imu_prop_timer = nh.createTimer(ros::Duration(0.004), &LIVMapper::imu_prop_callback, this);
  voxelmap_manager->voxel_map_pub_ = nh.advertise<visualization_msgs::MarkerArray>("/planes", 2);
}
...
void LIVMapper::handleVIO()
{
  ...
  // GLOBAL_LVBA_EXPORT_V1: export one accepted VIO frame.
  // Global-LVBA current source consumes image_poses.txt as T_W_I.
  if (global_lvba_output_en)
  {
    const double lvba_img_time = LidarMeasures.measures.back().vio_time;

    std::ostringstream ss_time;
    ss_time << std::fixed << std::setprecision(6) << lvba_img_time;

    const std::string image_path =
        global_lvba_output_dir + "/all_image/" +
        ss_time.str() + ".png";

    // Save the same image used by FAST-LIVO2 after any internal resize.
    // Keep it distorted/raw here; Global-LVBA performs its own undistortion.
    if (cv::imwrite(image_path, vio_manager->img_rgb))
    {
      Eigen::Quaterniond q_w_i(_state.rot_end);
      q_w_i.normalize();

      fout_lvba_image_pose
    ...
    {
      ROS_ERROR("[Global-LVBA] Failed to save image: %s",
                image_path.c_str());
    }
  }
  ...
  // RViz / PCD 已经使用完当前帧 pcl_w_wait_pub，释放其容量
...
void LIVMapper::handleLIO()
{
  ...
  // GLOBAL_LVBA_EXPORT_V1: export one LIO frame.
  // all_pcd_body is IMU/body frame: p_I = extR * p_L + extT.
  // lidar_poses.txt stores T_W_I.
  if (global_lvba_output_en &&
      feats_undistort &&
      !feats_undistort->empty())
  {
    const double lvba_lidar_time =
        LidarMeasures.measures.back().lio_time;

    std::ostringstream ss_time;
    ss_time << std::fixed << std::setprecision(6)
            << lvba_lidar_time;
    ...
    cloud_body->width = feats_undistort->size();
    cloud_body->height = 1;
    cloud_body->is_dense = feats_undistort->is_dense;

    for (size_t i = 0; i < feats_undistort->size(); ++i)
    {
      const auto &src = feats_undistort->points[i];

      V3D p_lidar(src.x, src.y, src.z);

      // LiDAR frame -> IMU/body frame
      V3D p_body = extR * p_lidar + extT;

      cloud_body->points[i].x = p_body.x();
      cloud_body->points[i].y = p_body.y();
      cloud_body->points[i].z = p_body.z();
      cloud_body->points[i].intensity = src.intensity;
    }

    // Write pose only after PCD save succeeds -> strict 1:1 PCD/pose count.
    if (pcl::io::savePCDFileBinary(pcd_path, *cloud_body) == 0)
    {
      Eigen::Quaterniond q_w_i(_state.rot_end);
      q_w_i.normalize();

      fout_lvba_lidar_pose
          << std::fixed << std::setprecision(9)
          << lvba_lidar_time << " "
          << _state.pos_end.x() << " "
          << _state.pos_end.y() << " "
    ...
    {
      ROS_ERROR("[Global-LVBA] Failed to save PCD: %s",
                pcd_path.c_str());
    }
  }
  ...
  *pcl_w_wait_pub = *laserCloudWorld;
  ...
  PointCloudXYZI().swap(*feats_down_body);
  PointCloudXYZI().swap(*feats_down_world);
  // PointCloudXYZI().swap(*pcl_w_wait_pub);
  // PointCloudXYZI().swap(*pcl_wait_pub);
}
...
void LIVMapper::standard_pcl_cbk(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
  ...
  while (lid_raw_data_buffer.size() > 10)
  {
...
void LIVMapper::InitializeRTK()
{
  ...
  for (const auto &rtk_data : rtk_buffer)
  {
    ...
    if (livo_it == livo_state_buffer.end())
      break;
...
void LIVMapper::imu_cbk(const sensor_msgs::Imu::ConstPtr &msg_in)
{
  ...
  if (imu_prop_enable)
  {
    ...
    if (imu_prop_enable && !p_imu->imu_need_init)
    {
      ...
      while (prop_imu_buffer.size() > 5000)
      {
        prop_imu_buffer.pop_front();
      }
    }
...
bool LIVMapper::sync_packages(LidarMeasureGroup &meas)
{
  ...
  case ONLY_LIO:
  {
    ...
    if (!img_en)
      keyframe_time = meas.lidar_frame_end_time;
    return true;
  ...
  case LIVO:
  {
    ...
    case VIO:
    {
      ...
      if (img_en)
        keyframe_time = img_capture_time;
      ...
      if (rtk_en && rtk_ini)
      {
        while (!rtk_buffer.empty())
        {
          ...
          if (rtk_buffer.front().timestamp > m.lio_time)
            break;
      ...
      *(meas.pcl_proc_cur) = *(meas.pcl_proc_next);
      ...
      const int max_lidar_frames_for_reserve = 5;
      int lid_frame_num = std::min<int>(lid_raw_data_buffer.size(), max_lidar_frames_for_reserve);
      int max_size = meas.pcl_proc_cur->size() + 24000 * lid_frame_num;
      max_size = std::min(max_size, 200000);
