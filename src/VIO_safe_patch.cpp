...
{
  // Safe version: avoid segmentation fault when projected pixel is close to image border
  // or when img is empty / non-continuous assumptions are invalid.
  if (patch_tmp == nullptr)
    return;

  const int offset = patch_size_total * level;
  std::fill(patch_tmp + offset, patch_tmp + offset + patch_size_total, 0.0f);
  ...
  const int img_step = static_cast<int>(img.step[0]);

  const float u_ref = pc[0];
  const float v_ref = pc[1];

  if (!std::isfinite(u_ref) || !std::isfinite(v_ref))
    return;
  ...
  // Bilinear interpolation below reads:
  // img_ptr[0], img_ptr[scale], img_ptr[scale * step], img_ptr[scale * step + scale].
  // Therefore the maximum accessed coordinate must leave one extra `scale` margin.
  const int min_u = u_ref_i - patch_size_half * scale;
  const int min_v = v_ref_i - patch_size_half * scale;
  const int max_u = min_u + (patch_size - 1) * scale + scale;
  const int max_v = min_v + (patch_size - 1) * scale + scale;

  if (min_u < 0 || min_v < 0 || max_u >= img_width || max_v >= img_height)
    return;
  ...
  {
    uint8_t *img_ptr = (uint8_t *)img.data + (min_v + x * scale) * img_step + min_u;
    for (int y = 0; y < patch_size; y++, img_ptr += scale)
    {
      patch_tmp[offset + x * patch_size + y] =
          w_ref_tl * img_ptr[0] +
...
void VIOManager::pruneVisualSparseMap()
{
  if (!visual_map_prune_en)
    return;
  ...
  const double keep_radius2 = visual_map_keep_radius * visual_map_keep_radius;

  size_t before_voxels = feat_map.size();
  size_t before_points = 0;
  size_t removed_voxels = 0;
  size_t removed_points = 0;

  std::vector<VOXEL_LOCATION> delete_keys;
  delete_keys.reserve(feat_map.size() / 10 + 1);
  ...
  for (auto &kv : feat_map)
  {
    ...
    if (voxel == nullptr)
    {
      delete_keys.push_back(kv.first);
      continue;
    }

    auto &pts = voxel->voxel_points;
    before_points += pts.size();

    bool keep_this_voxel = false;

    for (VisualPoint *pt : pts)
    {
      if (pt == nullptr)
        continue;

      double dist2 = (pt->pos_ - cur_pos).squaredNorm();

      if (dist2 <= keep_radius2)
      {
        keep_this_voxel = true;
        break;
      }
    }
  ...
  printf("[ VIO ] Prune visual sparse map: voxels %zu -> %zu, points %zu, removed_voxels: %zu, removed_points: %zu\n",
         before_voxels,
         feat_map.size(),
         before_points,
         removed_voxels,
...
void VIOManager::retrieveFromVisualSparseMap(cv::Mat img, vector<pointWithVar> &pg, const unordered_map<VOXEL_LOCATION, VoxelOctoTree *> &plane_map)
{
  ...
  for (auto &iter : sub_feat_map)
  {
    ...
    if (corre_voxel != feat_map.end() && corre_voxel->second != nullptr)
    {
  ...
  if (raycast_en)
  {
    for (int i = 0; i < length; i++)
    {
      ...
      for (const auto &it : rays_with_sample_points[i])
      {
        ...
        if (corre_feat_map != feat_map.end() && corre_feat_map->second != nullptr)
        {
          bool voxel_in_fov = false;
        ...
        else
        {
          ...
          if (iter != plane_map.end())
          {
            ...
            if (current_octo != nullptr && current_octo->plane_ptr_ != nullptr && current_octo->plane_ptr_->is_plane_)
            {
  ...
  for (int i = 0; i < length; i++)
  {
    if (grid_num[i] == TYPE_MAP)
    {
      ...
      if (!std::isfinite(pt->pos_[0]) || !std::isfinite(pt->pos_[1]) || !std::isfinite(pt->pos_[2]))
      {
        grid_num[i] = TYPE_UNKNOWN;
        continue;
      }
      ...
      V2D pc(new_frame_->w2c(pt->pos_));
      if (!std::isfinite(pc[0]) || !std::isfinite(pc[1]) || !new_frame_->cam_->isInFrame(pc.cast<int>(), border))
      {
        grid_num[i] = TYPE_UNKNOWN;
        continue;
      }
      ...
      if (ref_ftr == nullptr || ref_ftr->patch_ == nullptr || ref_ftr->img_.empty())
        continue;
...
void VIOManager::precomputeReferencePatches(int level)
{
  ...
  for (int i = 0; i < total_points; i++)
  {
    ...
    VisualPoint *pt = visual_submap->voxel_points[i];
    if (pt == nullptr || pt->ref_patch == nullptr)
      continue;
    ...
    const int min_u = u_ref_i - patch_size_half * scale;
    const int min_v = v_ref_i - patch_size_half * scale;
    const int max_u = min_u + (patch_size - 1) * scale + scale * 2;
    const int max_v = min_v + (patch_size - 1) * scale + scale * 2;
    if (!std::isfinite(u_ref) || !std::isfinite(v_ref) ||
        min_u - scale < 0 || min_v - scale < 0 || max_u >= img_width || max_v >= img_height)
      continue;
...
void VIOManager::updateStateInverse(cv::Mat img, int level)
{
  ...
  for (int iteration = 0; iteration < max_iterations; iteration++)
  {
    ...
    for (int i = 0; i < total_points; i++)
    {
      ...
      pc = cam->world2cam(pf);
      if (pf[2] <= 0 || !std::isfinite(pc[0]) || !std::isfinite(pc[1]) || !new_frame_->cam_->isInFrame(pc.cast<int>(), border))
        continue;
      ...
      const int max_u = min_u + (patch_size - 1) * scale + scale;
      const int max_v = min_v + (patch_size - 1) * scale + scale;
      if (img.empty() || img.data == nullptr || min_u < 0 || min_v < 0 || max_u >= img_width || max_v >= img_height)
        continue;
...
#pragma omp parallel for reduction(+ : error, n_meas)
    ...
    for (int i = 0; i < total_points; i++)
    {
      ...
      const int min_u = u_ref_i - patch_size_half * scale;
      const int min_v = v_ref_i - patch_size_half * scale;
      // Gradient calculation reads +/- scale and +2*scale around img_ptr.
      const int max_u = min_u + (patch_size - 1) * scale + scale * 2;
      const int max_v = min_v + (patch_size - 1) * scale + scale * 2;
      if (img.empty() || img.data == nullptr || min_u - scale < 0 || min_v - scale < 0 || max_u >= img_width || max_v >= img_height)
        continue;
...
void VIOManager::processFrame(cv::Mat &img, vector<pointWithVar> &pg, const unordered_map<VOXEL_LOCATION, VoxelOctoTree *> &feat_map, double img_time)
{
  ...
  // 每隔 visual_map_prune_interval 帧清理一次视觉稀疏地图，避免 feat_map 无限增长
  if (visual_map_prune_en &&
      visual_map_prune_interval > 0 &&
      frame_count > 0 &&
      frame_count % visual_map_prune_interval == 0)
  {
    pruneVisualSparseMap();
  }
